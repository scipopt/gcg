...
#define heurCopyMastervecldiving NULL

/** destructor of primal heuristic to free user data (called when SCIP is exiting) */
static
SCIP_DECL_HEURFREE(heurFreeMastervecldiving) /*lint --e{715}*/
{  /*lint --e{715}*/
...
static
SCIP_DECL_HEURINIT(heurInitMastervecldiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitMastervecldiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecMastervecldiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP* origprob;
   SCIP_HEURDATA* heurdata;
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIPdebugMessage("(node %"SCIP_LONGINT_FORMAT") executing mastervecldiving heuristic: depth=%d, %d fractionals, dualbound=%g, searchbound=%g\n",
      ...
      && !SCIPisStopped(scip) )
   {
      ...
      if( allroundable )
      {
         ...
         if( success )
         {
            SCIPdebugMessage("mastervecldiving found roundable primal solution: obj=%g\n", SCIPgetSolOrigObj(scip, heurdata->sol));
   ...
   if( nlpcands == 0 && !lperror && !cutoff && lpsolstat == SCIP_LPSOLSTAT_OPTIMAL )
   {
      ...
      SCIP_CALL( SCIPlinkLPSol(scip, heurdata->sol) );
      SCIPdebugMessage("mastervecldiving found primal solution: obj=%g\n", SCIPgetSolOrigObj(scip, heurdata->sol));
   ...
   SCIPdebugMessage("mastervecldiving heuristic finished\n");
   ...
   )
{
   ...
   /* create Mastervecldiving primal heuristic data */
   SCIP_CALL( SCIPallocMemory(scip, &heurdata) );

   /* include primal heuristic */
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecMastervecldiving, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeMastervecldiving) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitMastervecldiving) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitMastervecldiving) );

   /* mastervecldiving heuristic parameters */
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/mastervecldiving/minreldepth",
         "minimal relative depth to start diving",
         &heurdata->minreldepth, TRUE, DEFAULT_MINRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/mastervecldiving/maxreldepth",
         "maximal relative depth to start diving",
         &heurdata->maxreldepth, TRUE, DEFAULT_MAXRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/mastervecldiving/maxlpiterquot",
         "maximal fraction of diving LP iterations compared to node LP iterations",
         &heurdata->maxlpiterquot, FALSE, DEFAULT_MAXLPITERQUOT, 0.0, SCIP_REAL_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/mastervecldiving/maxlpiterofs",
         "additional number of allowed LP iterations",
         &heurdata->maxlpiterofs, FALSE, DEFAULT_MAXLPITEROFS, 0, INT_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/mastervecldiving/maxpricequot",
