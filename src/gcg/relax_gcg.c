   ...
   )
{
   ...
   SCIP_CALL( GCGdecompAddRemainingConss(scip, decomp) );
   ...
   )
{
   ...
   for( i = 0; i < relaxdata->nbranchrules; i++ )
   {
      if( branchrule == relaxdata->branchrules[i]->branchrule )
      {
         ...
         if( relaxdata->branchrules[i]->branchgetmastercut != NULL )
         {
...
SCIP_RETCODE GCGrelaxIncludeSeparator(
   ...
   GCG_DECL_SEPAGETMASTERCUTDATA    ((*gcgsepagetmastercutdata)),  /**< get the mastercut data of row */
   GCG_DECL_SEPAUPDATENEWCUTS       ((*gcgsepaupdatenewcuts)),     /**< makes new cuts available */
   GCG_DECL_SEPAGETCUTS             ((*gcgsepagetcuts)),           /**< get all relevant cuts generated by this separator */
   ...
   )
{
   ...
   relaxdata->separators[pos]->gcgsepagetmastercutdata = gcgsepagetmastercutdata;
   relaxdata->separators[pos]->gcgsepaupdatenewcuts = gcgsepaupdatenewcuts;
   relaxdata->separators[pos]->gcgsepagetcuts = gcgsepagetcuts;
...
/** returns the total number of relevant cuts generated by gcg separators */
...
)
{
   ...
   ntotalcuts = 0;
   for( i = 0; relaxdata->nseparators; i++ )
   {
      if( relaxdata->separators[i]->gcgsepagetcuts != NULL )
      {
         relaxdata->separators[i]->gcgsepagetcuts(scip, relaxdata->separators[i], &mastercutdata, &nmastercutdata);
         ntotalcuts += nmastercutdata;
      }
   }
   return ntotalcuts;
}

/** updates all the gcg separators registered with the master problem */
   ...
   )
{
   ...
   for( i = 0; i < relaxdata->nseparators; i++ )
   {
      if( relaxdata->separators[i]->gcgsepaupdatenewcuts != NULL )
      {
         SCIPdebugMessage("update gcg separator %s\n", SCIPsepaGetName(relaxdata->separators[i]->separator));
         SCIP_CALL( relaxdata->separators[i]->gcgsepaupdatenewcuts(scip, relaxdata->separators[i]) );
      }
   }
