...
SCIP_RETCODE ObjPricerGcg::setPricingObjs(
   ...
   GCG_MASTERCUTDATA**   branchmastercutdata,/**< array of mastercuts generated by branching in all currently active nodes */
   int                   nbranchmastercuts   /**< number of currently active branching rules that created mastercuts */
   )
{
   ...
   for( i = 0; i < nbranchmastercuts; i++ )
   {
      ...
      SCIP_CALL( GCGrelaxBranchUpdateDualWithGCGBranchrule(scip_, branchrules[i], branchdata[i], dualsol) );
   }
   ...
   )
{
   ...
   *pnfoundvars = 0;
   ...
   do
   {
      ...
      *bestredcostvalid = isMasterLPOptimal() && !GCGisBranchruleGeneric(GCGconsMasterbranchGetBranchrule(GCGconsMasterbranchGetActiveCons(scip_)));
      ...
      /** @todo-mastercuts: this should take all the mastercuts (all active ones from branching + all (in-)active from separators) */
      SCIP_CALL( setPricingObjs(pricetype, stabilized, branchrules, branchdata, branchmastercutdata, nbranchmastercuts) );
   ...
   *pnfoundvars = nfoundvars;
   ...
   // free branchrules and -data
   if( nbranchmastercuts>0 )
   {
      SCIPfreeBlockMemoryArrayNull(scip_, &branchrules, nbranchmastercuts);
      SCIPfreeBlockMemoryArrayNull(scip_, &branchdata, nbranchmastercuts);
      SCIPfreeBlockMemoryArrayNull(scip_, &branchmastercutdata, nbranchmastercuts);
      branchrules = NULL;
      branchdata = NULL;
      branchmastercutdata = NULL;
   }
...
)
{
   ...
   branchmastercutdata = NULL;
   nbranchmastercuts = 0;
   SCIP_CALL( GCGrelaxBranchGetAllActiveMasterCuts(scip, branchrules, branchdata, branchmastercutdata, &nbranchmastercuts) );
   ...
   SCIP_CALL( pricer->setPricingObjs(pricer->getReducedCostPricingNonConst(), FALSE, branchrules, branchdata, branchmastercutdata, nbranchmastercuts) );
   ...
   if( nbranchmastercuts>0 )
   {
      SCIPfreeBlockMemoryArrayNull(scip, &branchrules, nbranchmastercuts);
      SCIPfreeBlockMemoryArrayNull(scip, &branchdata, nbranchmastercuts);
      SCIPfreeBlockMemoryArrayNull(scip, &branchmastercutdata, nbranchmastercuts);
   }
