...
)
{
   ...
   for( j = ncurrentsepamastercutcoeffs; j < nactivecuts; j++ )
   {
      ...
      if( GCGmastercutIsActive(mastercutdata) )
         SCIP_CALL( sepas[GCGmastersepacutGetSeparatorIndex(activecuts[j])]->gcgsepagetcolcoefficient(scip_, sepas[GCGmastersepacutGetSeparatorIndex(activecuts[j])], activecuts[j], gcgcol, &coeffs[j]) );
   }
