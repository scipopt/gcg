   ...
   )
{
   ...
   (*gcgcol)->mastersepacutcoeffs = NULL;
   (*gcgcol)->nmastersepacutcoeffs = 0;
   (*gcgcol)->mastersepacutscoeffssize = 0;
   ...
   )
{
   ...
   SCIPfreeBlockMemoryArrayNull((*gcgcol)->pricingprob, &((*gcgcol)->mastersepacutcoeffs), (*gcgcol)->mastersepacutscoeffssize);
   (*gcgcol)->mastersepacutscoeffssize =0;
   (*gcgcol)->nmastersepacutcoeffs = 0;
   ...
   )
{
   ...
   for( j = 0; j < gcgcol->nmastersepacutcoeffs; j++ )
   {
      if( !SCIPisZero(scip, gcgcol->mastersepacutcoeffs[j]) )
         norm += SQR(gcgcol->mastersepacutcoeffs[j]);;
   }
...
{
   return gcgcol->nmastersepacutcoeffs;
}
...
{
   return gcgcol->mastersepacutscoeffssize;
}
...
{
   return gcgcol->mastersepacutcoeffs;
}
   ...
   *para = 0.0;
   ...
   for( j = 0; j < gcgcol->nmastersepacutcoeffs; j++ )
   {
      ...
      if( !SCIPisInfinity(masterprob, -lhs) )
      {
         ...
         if( SCIPisPositive(masterprob, gcgcol->mastersepacutcoeffs[j]) )
            *para += gcgcol->mastersepacutcoeffs[j] * lhs;
      }
      else if( !SCIPisInfinity(masterprob, rhs) )
      {
         dualobjnorm += SQR(rhs);

         if( SCIPisNegative(masterprob, gcgcol->mastersepacutcoeffs[j]) )
            *para += gcgcol->mastersepacutcoeffs[j] * rhs;
      }
   }
   ...
   )
{
   ...
   /* consider cuts generated by separators */
   assert(gcgcol1->nmastersepacutcoeffs == gcgcol2->nmastersepacutcoeffs);
   for( j = 0; j < gcgcol1->nmastersepacutcoeffs; j++ )
   {
      if( SCIPisPositive(scip, gcgcol1->mastersepacutcoeffs[j] * gcgcol2->mastersepacutcoeffs[j]) )
         para += gcgcol1->mastersepacutcoeffs[j] * gcgcol2->mastersepacutcoeffs[j];

      if( SCIPisPositive(scip, gcgcol1->mastersepacutcoeffs[j]) )
         norm1 += SQR(gcgcol1->mastersepacutcoeffs[j]);

      if( SCIPisPositive(scip, gcgcol2->mastersepacutcoeffs[j]) )
         norm2 += SQR(gcgcol2->mastersepacutcoeffs[j]);
   }
