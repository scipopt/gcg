...
struct SCIP_ConsData
{
   ...
   /* information needed to update rows generated by separators */
   GCG_MASTERSEPACUT***  addedcuts;          /**< master sepa cuts which were added in this node for each sepa */
   int*                  naddedcuts;         /**< number of master sepa cuts which were added in this node for each sepa */
   int*                  firstnewcut;        /**< index of first cut added at this node for each sepa */
   ...
   )
{
   ...
   if( nsepas > 0 )
   {
      ...
      for( i = 0; i < nsepas; i++ )
      {
         assert(consdata->firstnewcut[i] == nactivecuts[i]);

         /* store the current number of active cuts */
         consdata->firstnewcut[i] = nactivecuts[i];
...
/** stores the relevant cuts in node, if they have not been stored yet*/
static
   ...
   )
{
   ...
   if( conshdlrdata->nsepas > 0 )
   {
      ...
      {
#ifndef MASTERSEP_DEBUG
         SCIPinfoMessage(scip, NULL, "init added cuts: node %lli of type %i stores no cuts\n",
                         SCIPnodeGetNumber(consdata->node), SCIPnodeGetType(consdata->node));
#endif
      ...
      for( i = 0; i < conshdlrdata->nsepas; i++ )
      {
         ...
         assert(consdata->firstnewcut[i] <= nactivecuts[i]);
         if( consdata->firstnewcut[i] == nactivecuts[i] )
         {
#ifndef MASTERSEP_DEBUG
            SCIPinfoMessage(scip, NULL, "init consdata: no cuts to store for sepa %i\n", i);
#endif
            continue;
         }
         consdata->naddedcuts[i] = nactivecuts[i] - consdata->firstnewcut[i];
#ifndef MASTERSEP_DEBUG
         ...
         for( j = 0; j < consdata->naddedcuts[i]; j++ )
         {
            SCIP_CALL( GCGcaptureMasterSepaCut(consdata->addedcuts[i][j]) );
         }
      }
   }
   consdata->addedcutsinit = TRUE;
...
/*    assert((SCIPgetNNodesLeft(scip)+SCIPgetNNodes(scip) == 1) == (consdata->node == SCIPgetRootNode(scip))); */
   ...
   /* if separators are registered: store the number of cuts stored before node is solved */
   if( conshdlrdata->nsepas > 0 )
   {
      SCIP_CALL( SCIPduplicateBlockMemoryArray(scip, &(consdata->firstnewcut), GCGgetNActiveCuts(scip), conshdlrdata->nsepas) );
#ifndef MASTERSEP_DEBUG
      SCIPinfoMessage(scip, NULL, "init cons data: store current number of active cuts for node %lli\n", SCIPnodeGetNumber(consdata->node));
      for( i = 0; i < conshdlrdata->nsepas; i++ )
      {
         SCIPinfoMessage(scip, NULL, "init cons data: firstnewcut[%i] = %i\n", i, consdata->firstnewcut[i]);
      }
#endif
   }
   ...
   if( nsepas > 0 && (*consdata)->addedcutsinit )
   {
      ...
      if( (*consdata)->nodestoredcuts )
      {
         for( i = 0; i < nsepas; i++ )
         {
            ...
            for( j = 0; j < (*consdata)->naddedcuts[i]; j++ )
            {
               SCIP_CALL( GCGreleaseMasterSepaCut(scip, &((*consdata)->addedcuts[i][j])) );
            }
