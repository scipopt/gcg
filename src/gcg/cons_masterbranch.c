...
/* adds (if needed) master variable to the rows which were generated by separators */
static
SCIP_RETCODE updateAddedRows(
   SCIP* scip,                /**< SCIP data structure (master) */
   SCIP_ROW** addedrows,      /**< rows which need to be updated */
   int naddedrows,            /**< number of rows which need to be updated */
   SCIP_VAR** pricingvars,    /**< pricing variables needed to compute coefficient for master variable */
   SCIP_Real* pricingvals,    /**< values of the column which generated the master variable */
   int npricingvars,          /**< number of pricing variables */
   SCIP_VAR* mastervar        /**< master variable which is added to rows */
)
{
   ...
   ngcgsepas = GCGrelaxGetNSeparators(scip);
   gcgsepas = GCGrelaxGetSeparators(scip);

   for( i = 0; i < naddedrows; i++ )
   {
      /* we skip rows which were not generated by separators */
      sepa = SCIProwGetOriginSepa(addedrows[i]);
      if( sepa != NULL )
         continue;

      SCIPdebugMessage("--> Row %s (generated by separator %s, ", SCIProwGetName(addedrows[i]), SCIPsepaGetName(sepa));
      /* global cuts can be added to the LP in multiple nodes: these variables might already be in the cut */
      if( !SCIProwIsLocal(addedrows[i]) )
      {
         SCIPdebugMessage("global, ");
         if( SCIProwIsInLP(addedrows[i]) ) // global cut is already up-to-date
            continue;
         SCIPdebugMessage("not in LP)");
         /* @todo: check if variable is already in the global cut, else add it */
      }
      else
      {
         SCIPdebugMessage("local) ");
         /* find the separator which generated this cut and update it */
         for( j = 0; j < ngcgsepas; j++ )
         {
            if( gcgsepas[j]->separator == sepa )
            {
               if( gcgsepas[j]->gcgsepagetmastercutdata == NULL )
                  break;

               SCIP_CALL( gcgsepas[j]->gcgsepagetmastercutdata(scip, gcgsepas[j], addedrows[i], &mastercutdata) );
               if( mastercutdata != NULL )
               {
                  SCIP_CALL( gcgsepas[j]->gcgsepagetvarcoefficient(scip, gcgsepas[i], mastercutdata, pricingvars, pricingvals, npricingvars, &coef) );
                  SCIP_CALL( SCIPaddVarToRow(scip, addedrows[j], mastervar, coef) );
                  SCIPdebugMessage("was successfully updated.\n");
               }
               break;
            }
         }
...
/* adds (if needed) master variable to the rows which were generated by separators at the node */
static
SCIP_RETCODE updateNodeRows(
   SCIP* scip,             /**< SCIP data structure (master) */
   SCIP_NODE* node,        /**< node whose added rows should be updated */
   SCIP_VAR* mastervar     /**< master variable which is added */
)
{
   SCIP_VAR** pricingvars;
   SCIP_ROW** addedrows;
   ...
   SCIPdebugMessage("Update rows at node %lli with master variable %s\n", SCIPnodeGetNumber(node), SCIPvarGetName(mastervar));
   if( (SCIPnodeGetType(node) == SCIP_NODETYPE_PSEUDOFORK) || (SCIPnodeGetType(node) == SCIP_NODETYPE_FORK) )
   {
      /* retrieve the pricing vars corresponding to the mastervar and their values */
      npricingvars = mastervar->vardata->data.mastervardata.norigvars;
      SCIP_CALL( SCIPallocBufferArray(scip, &pricingvars, npricingvars) );
      SCIP_CALL( SCIPallocBufferArray(scip, &pricingvals, npricingvars) );
      for( i = 0; i < npricingvars; i++ )
      {
         pricingvars[i] = mastervar->vardata->data.mastervardata.origvars[i]->vardata->data.origvardata.pricingvar;
         pricingvals[i] = mastervar->vardata->data.mastervardata.origvals[i];
      }

      if( SCIPnodeGetType(node) == SCIP_NODETYPE_PSEUDOFORK )
      {
         naddedrows = node->data.pseudofork->naddedrows;
         addedrows = node->data.pseudofork->addedrows;
      }
      else
      {
         naddedrows = node->data.fork->naddedrows;
         addedrows = node->data.fork->addedrows;
      }

      SCIP_CALL( updateAddedRows(scip, addedrows, naddedrows, pricingvars, pricingvals, npricingvars, mastervar) );
   ...
   else
   {
      ...
      do
      {
         ...
         for( i = consdata->knownvarhistory->pos + 1; i < consdata->knownvarhistory->buffer->nvars; i++ )
         {
            ...
            SCIP_CALL( updateNodeRows(scip, consdata->node, consdata->knownvarhistory->buffer->vars[i]) );
         }
...
SCIP_DECL_CONSACTIVE(consActiveMasterbranch)
{
   ...
   SCIPinfoMessage(scip, NULL, "Activate node %lli of type %i in %s\n", consdata->node->number, SCIPnodeGetType(consdata->node),
...
SCIP_DECL_CONSDEACTIVE(consDeactiveMasterbranch)
{
   ...
   SCIPinfoMessage(scip, NULL, "Deactivate node %lli of type %i in prob %s\n", consdata->node->number, SCIPnodeGetType(consdata->node), SCIPgetProbName(scip));
...
SCIP_DECL_CONSDELETE(consDeleteMasterbranch)
{
   ...
   assert(strcmp(SCIPconshdlrGetName(conshdlr), CONSHDLR_NAME) == 0);
   assert(*consdata != NULL);
   //SCIPinfoMessage(scip, NULL, "Delete node %lli of type %i in %s\n", (*consdata)->node->number, SCIPnodeGetType((*consdata)->node), SCIPgetProbName(scip));
