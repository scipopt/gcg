...
#define heurCopyGcglinesdiving NULL

/** destructor of primal heuristic to free user data (called when SCIP is exiting) */
static
SCIP_DECL_HEURFREE(heurFreeGcglinesdiving)
{  /*lint --e{715}*/
...
static
SCIP_DECL_HEURINIT(heurInitGcglinesdiving)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitGcglinesdiving)
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
#define heurInitsolGcglinesdiving NULL


/** solving process deinitialization method of primal heuristic (called before branch and bound process data is freed) */
#define heurExitsolGcglinesdiving NULL


/** execution method of primal heuristic */
static
SCIP_DECL_HEUREXEC(heurExecGcglinesdiving)
{  /*lint --e{715}*/
   SCIP* masterprob;
   SCIP_HEURDATA* heurdata;
   ...
   *result = SCIP_DELAYED;
   ...
   {
      SCIPdebugMessage("not executing GCG linesearchdiving: invalid relaxation solution (should not happen!)\n");
      return SCIP_OKAY;
   }
...
//      maxpricerounds = (1.0 + 10.0*(nsolsfound+1.0)/(ncalls+1.0)) * npricerounds;
   ...
   *result = SCIP_DIDNOTFIND;
	  ...
	  && !SCIPisStopped(scip) )
   {
      ...
      if( success )
      {
         SCIPdebugMessage("GCG linesearchdiving found roundable primal solution: obj=%g\n", SCIPgetSolOrigObj(scip, heurdata->sol));
...
//               SCIP_CALL( SCIPtrySol(scip, heurdata->sol, FALSE, FALSE, FALSE, FALSE, &success) );
      ...
      do
      {
         ...
         if( !cutoff )
         {
            ...
            { 
               SCIPwarningMessage("Error while solving LP in GCG linesearchdiving heuristic; LP solve terminated with code <%d>\n",retstat);
            }
#else
...
//      SCIP_CALL( SCIPtrySol(scip, heurdata->sol, FALSE, FALSE, FALSE, FALSE, &success) );
   ...
   SCIPdebugMessage("(node %"SCIP_LONGINT_FORMAT") finished GCG linesearchdiving heuristic: %d fractionals, dive %d/%d, LP iter %"SCIP_LONGINT_FORMAT"/%"SCIP_LONGINT_FORMAT", pricerounds %d/%d, objval=%g/%g, lpsolstat=%d, cutoff=%u\n",
   ...
   )
{
   ...
   /* create GCG linesearchdiving primal heuristic data */
   SCIP_CALL( SCIPallocMemory(scip, &heurdata) );

   /* include primal heuristic */
   SCIP_CALL( SCIPincludeHeur(scip, HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP,
         heurCopyGcglinesdiving, heurFreeGcglinesdiving, heurInitGcglinesdiving, heurExitGcglinesdiving,
         heurInitsolGcglinesdiving, heurExitsolGcglinesdiving, heurExecGcglinesdiving,
         heurdata) );

   /* add GCG linesearchdiving primal heuristic parameters */
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcglinesdiving/minreldepth",
         "minimal relative depth to start diving",
         &heurdata->minreldepth, TRUE, DEFAULT_MINRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcglinesdiving/maxreldepth",
         "maximal relative depth to start diving",
         &heurdata->maxreldepth, TRUE, DEFAULT_MAXRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcglinesdiving/maxlpiterquot",
         "maximal fraction of diving LP iterations compared to node LP iterations",
         &heurdata->maxlpiterquot, FALSE, DEFAULT_MAXLPITERQUOT, 0.0, SCIP_REAL_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcglinesdiving/maxlpiterofs",
         "additional number of allowed LP iterations",
         &heurdata->maxlpiterofs, FALSE, DEFAULT_MAXLPITEROFS, 0, INT_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcglinesdiving/maxpricequot",
