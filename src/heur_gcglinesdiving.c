...
//               SCIP_CALL( SCIPtrySol(scip, heurdata->sol, FALSE, FALSE, FALSE, FALSE, &success) );
      ...
      do
      {
         ...
         if( !cutoff || farkaspricing )
         {
            ...
            {
               SCIPwarningMessage(scip, "Error while solving LP in GCG linesearchdiving heuristic; LP solve terminated with code <%d>\n",retstat);
            }
#else
            if( (!heurdata->usefarkasonly || farkaspricing)
               && (heurdata->maxpricerounds == -1 || totalpricerounds < heurdata->maxpricerounds) )
   ...
   )
{
   ...
   /* create gcglinesearchdiving primal heuristic data */
   SCIP_CALL( SCIPallocMemory(scip, &heurdata) );

   /* include primal heuristic */
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
      HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
      HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecGcglinesdiving, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeGcglinesdiving) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitGcglinesdiving) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitGcglinesdiving) );
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcglinesdiving/maxpricerounds",
   ...
   SCIP_CALL( SCIPaddBoolParam(scip,
         "heuristics/gcglinesdiving/usefarkasonly",
