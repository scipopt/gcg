...
#define HEUR_NAME             "clasfeaspump"
#define HEUR_DESC             "(classical) objective feasibility pump 2.0, using the GCG relaxator"
#define HEUR_DISPCHAR         'F'
#define HEUR_PRIORITY         -1000000
#define HEUR_FREQ             -1 /* 30 */
...
struct SCIP_HeurData
{
   ...
   SCIP_Real*            origobjs;           /**< diving objective values of original variables */
...
//   SCIP_CALL( SCIPcopy(scip, *probingscip, *varmapfw, NULL, "feaspump", FALSE, FALSE, success) );
   ...
   /* copy all plugins */
   SCIP_CALL( SCIPincludeDefaultPlugins(*probingscip) );

   /* create the subproblem */
   SCIP_CALL( SCIPcreateProb(*probingscip, "feaspump", NULL, NULL, NULL, NULL, NULL, NULL, NULL) );

   /* copy all variables */
   SCIP_CALL( SCIPcopyVars(scip, *probingscip, *varmapfw, NULL, FALSE) );

   /* copy all constraints
    * TODO: SCIPcopyConss() doesn't seem to work properly here, therefore, copy the rows manually */
//   SCIP_CALL( SCIPcopyConss(scip, *probingscip, *varmapfw, NULL, FALSE, FALSE, success) );
//   SCIPdebugMessage("copying the SCIP constraints was %s complete.\n", *success ? "" : "not ");
   ...
   for( i = 0; i < nrows; i++ )
   {
      ...
      for( j = 0; j < nnonz; j++ )
         consvars[j] =  SCIPhashmapGetImage(*varmapfw, SCIPcolGetVar(cols[j]));

      /* create a new linear constraint and add it to the subproblem */
      SCIP_CALL( SCIPcreateConsLinear(*probingscip, &cons, SCIProwGetName(rows[i]), nnonz, consvars, vals, lhs, rhs,
   ...
   )
{
   ...
   for( i = 0; i < nflipcands; i++ )
   {
      ...
      heurdata->origobjs[SCIPvarGetProbindex(var)] = newobjcoeff;
   }
   return SCIP_OKAY;
}
...
static
SCIP_DECL_HEURCOPY(heurCopyClasfeaspump)
{
   assert(scip != NULL);
   assert(heur != NULL);
...
static
SCIP_DECL_HEURFREE(heurFreeClasfeaspump)
{
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINIT(heurInitClasfeaspump)
{
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitClasfeaspump)
{
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEURINITSOL(heurInitsolClasfeaspump)
{
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXITSOL(heurExitsolClasfeaspump)
{
...
static
SCIP_DECL_HEUREXEC(heurExecClasfeaspump)
{
   SCIP* masterprob;          /* GCG master problem */
   SCIP_HEURDATA* heurdata; 
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   {
      SCIPdebugMessage("not executing classical feaspump: invalid relaxation solution (should not happen!)\n");
      return SCIP_OKAY;
   }
...
//      return SCIP_OKAY;
   ...
   SCIPdebugMessage("executing classical feasibility pump heuristic, nlpiters=%"SCIP_LONGINT_FORMAT", maxnlpit:%"SCIP_LONGINT_FORMAT", maxflips:%d \n",
      nlpiterations, maxnlpiterations, maxflips);

   *result = SCIP_DIDNOTFIND;

   /* memory allocation */
   SCIP_CALL( SCIPallocBufferArray(scip, &heurdata->origobjs, nvars) );
      ...
      && !SCIPisStopped(scip) )
   {
      ...
      if( success )
      {
         SCIPdebugMessage("classical feaspump found roundable primal solution: obj=%g\n", SCIPgetSolOrigObj(scip, heurdata->sol));
//         SCIP_CALL( SCIPtrySol(scip, heurdata->sol, FALSE, FALSE, FALSE, FALSE, &success) );
      ...
      if( heurdata->usefp20 )
      {
         ...
         {
            frac = SCIPfeasFrac(scip, SCIPgetRelaxSolVal(scip, pseudocands[i]));
      ...
      {
         heurdata->origobjs[i] = alpha *  SCIPvarGetObj(vars[i]);
      }
...
#endif
         SCIPwarningMessage("Error while solving LP in classical Feaspump heuristic; LP solve terminated with code <%d>\n", retcode);
      ...
      if( heurdata->stage3 )
      {
         ...
         for( i = 0; i < nbinvars+nintvars; i++ )
         {
            ...
            lpval = SCIPgetRelaxSolVal(scip, vars[i]);
   ...
   if( nfracs == 0 && !lperror && lpsolstat == SCIP_LPSOLSTAT_OPTIMAL )
   {
      ...
      SCIP_CALL( SCIPlinkRelaxSol(scip, heurdata->sol) );
      SCIPdebugMessage("classical feaspump found primal solution: obj=%g\n", SCIPgetSolOrigObj(scip, heurdata->sol));
...
//      SCIP_CALL( SCIPtrySol(scip, heurdata->sol, FALSE, FALSE, FALSE, FALSE, &success) );
   ...
   if( heurdata->stage3 && (*result != SCIP_FOUNDSOL) && SCIPisLE(scip, mindistance, (SCIP_Real) heurdata->neighborhoodsize) )
   {
      ...
      SCIPdebugMessage("no solution found - entering stage 3\n");
      ...
      if( timelimit > 10.0 && memorylimit >= 0.0 )
      {
         ...
         if( SCIPgetNSols(probingscip) > 0 )
         {
            ...
            if( success )
            {
               SCIPdebugMessage(" -> stage 3 yielded feasible solution.\n");
               *result = SCIP_FOUNDSOL;
            }
   ...
   SCIPfreeBufferArray(scip, &heurdata->origobjs);
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeur(scip, HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         ...
         heurCopyClasfeaspump,
         heurFreeClasfeaspump, heurInitClasfeaspump, heurExitClasfeaspump,
         heurInitsolClasfeaspump, heurExitsolClasfeaspump, heurExecClasfeaspump,
         heurdata) );
