   ...
   )
{
   ...
   {
      var = mostfracvars[i];
      solval = GCGrelaxGetVarSol(scip, var);   
...
static
SCIP_DECL_HEURINITSOL(heurInitsolGcgfeaspump)
{
   SCIP_HEURDATA* heurdata;
   ...
   if( heurdata->beforecuts && SCIPheurGetFreqofs(heur) == 0 )
      SCIPheurSetTimingmask(heur, SCIP_HEURTIMING_AFTERPSEUDONODE);
...
static
SCIP_DECL_HEUREXITSOL(heurExitsolGcgfeaspump)
{
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DELAYED;
   ...
   if( SCIPgetLPSolstat(GCGrelaxGetMasterprob(scip)) != SCIP_LPSOLSTAT_OPTIMAL )
      return SCIP_OKAY;
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   /* only call gcgfeaspump once at the root */
   ...
   nfracs = GCGrelaxGetNBranchCands(scip);
   assert(0 <= nfracs && nfracs <= nbinvars + nintvars);
   if( nfracs == 0 )
      return SCIP_OKAY;

   /* calculate the maximal number of LP iterations until heuristic is aborted */
   nlpiterations = SCIPgetNLPIterations(GCGrelaxGetMasterprob(scip));
   ...
   *result = SCIP_DIDNOTFIND;
	  ...
	  && !SCIPisStopped(scip) )
   {
      ...
      SCIP_CALL( GCGrelaxLinkSol(scip, heurdata->sol) );
      ...
      SCIP_CALL( GCGrelaxLinkSol(scip, heurdata->roundedsol) );
       ...
       */
#ifdef NDEBUG
      retstat = SCIPsolveProbingLP(scip, iterlimit, &lperror);
      if( retstat != SCIP_OKAY )
      { 
         SCIPwarningMessage("Error while solving LP in Gcgfeaspump heuristic; LP solve terminated with code <%d>\n",retstat);
      }
#else
      SCIP_CALL( SCIPsolveProbingLP(scip, iterlimit, &lperror) );
#endif
   ...
   )
{
   ...
   SCIP_CALL( SCIPincludeHeur(scip, HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         ...
         heurFreeGcgfeaspump, heurInitGcgfeaspump, heurExitGcgfeaspump, 
         heurInitsolGcgfeaspump, heurExitsolGcgfeaspump, heurExecGcgfeaspump,
         heurdata) );

   /* add gcgfeaspump primal heuristic parameters */
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcgfeaspump/maxlpiterquot", 
         "maximal fraction of diving LP iterations compared to node LP iterations",
         &heurdata->maxlpiterquot, FALSE, DEFAULT_MAXLPITERQUOT, 0.0, SCIP_REAL_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcgfeaspump/maxlpiterofs", 
         "additional number of allowed LP iterations",
         &heurdata->maxlpiterofs, FALSE, DEFAULT_MAXLPITEROFS, 0, INT_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcgfeaspump/maxsols", 
         "total number of feasible solutions found up to which heuristic is called (-1: no limit)",
         &heurdata->maxsols, TRUE, DEFAULT_MAXSOLS, -1, INT_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/gcgfeaspump/objfactor", 
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcgfeaspump/minflips", 
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcgfeaspump/cyclelength", 
   ...
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/gcgfeaspump/perturbfreq", 
   ...
   SCIP_CALL( SCIPaddBoolParam(scip,
         "heuristics/gcgfeaspump/beforecuts", 
