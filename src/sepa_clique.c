...
#define SEPA_NAME              "cliquegcg"
#define SEPA_DESC              "clique separator of stable set relaxation for gcg"
   ...
   )
{
   ...
   maxnnodes = 2*SCIPgetNBinVars(GCGprobGetOrigprob(scip));
   assert(maxnnodes > 0);
   ...
   )
{
   ...
   {
      SCIP_CALL( SCIPreleaseVar(GCGprobGetOrigprob(scip), &(*tcliquegraph)->vars[v]) );
   }
   ...
   )
{
   ...
   assert(*tcliquegraph != NULL);
   assert((*tcliquegraph)->nnodes < 2*SCIPgetNBinVars(GCGprobGetOrigprob(scip)) - 1);

   /* if the value is FALSE, use the negated variable for the node */
   if( !value )
   {
      SCIP_CALL( SCIPgetNegatedVar(GCGprobGetOrigprob(scip), var, &nodevar) );
   }
   else
      nodevar = var;
   ...
   *nodeidx = (*tcliquegraph)->nnodes;
   SCIP_CALL( SCIPcaptureVar(GCGprobGetOrigprob(scip), nodevar) );
   ...
   )
{
   ...
   vars = SCIPgetVars(GCGprobGetOrigprob(scip));
   nvars = SCIPgetNBinVars(GCGprobGetOrigprob(scip));
   ...
   )
{
   ...
   for( i = 0; i < tcliquegraph->nnodes; ++i )
   {
      ...
      assert(0 <= SCIPvarGetProbindex(var) && SCIPvarGetProbindex(var) < SCIPgetNBinVars(GCGprobGetOrigprob(scip)));
      ...
      for( j = 0; j < nbinimpls; ++j )
      {
         ...
         assert(0 <= probidx && probidx < SCIPgetNBinVars(GCGprobGetOrigprob(scip)));
   ...
   )
{
   ...
   /* get clique table */
   cliques = SCIPgetCliques(GCGprobGetOrigprob(scip));
   ncliques = SCIPgetNCliques(GCGprobGetOrigprob(scip));
   if( ncliques == 0 )
      return SCIP_OKAY;
   ...
   SCIPdebugMessage("clique separator: constructing dense clique table (%d kb, %d cliques, %d nodes, density: %.2f)\n",
      tablesize/1024, SCIPgetNCliques(GCGprobGetOrigprob(scip)), tcliquegraph->nnodes, density);
   ...
   if( cliqueweight > sepadata->scaleval )
   {
      ...
      if( SCIPisEfficacious(scip, unscaledweight - 1.0) )
      {
         ...
         for( i = 0; i < ncliquenodes; ++i )
         {
            ...
            printf(" [%d]<%s>", cliquenodes[i], SCIPvarGetName(vars[cliquenodes[i]]));
         }
         /*SCIPdebugPrintf("\n");*/
         printf("\n");
...
{  /*lint --e{715}*/
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIP_CALL( GCGgetSolVals(scip, sol, tcliquegraph->nnodes, tcliquegraph->vars, sepadata->varsolvals) );
...
SCIP_RETCODE SCIPincludeSepaCliqueGCG(
   SCIP*                 scip                /**< SCIP data structure */
   )
{
   SCIP_SEPADATA* sepadata;
