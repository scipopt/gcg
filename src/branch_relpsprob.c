 ...
 */
static
SCIP_RETCODE SCIPgetLPBranchCandsInt(
   SCIP*                 scip,               /**< SCIP data structure */
   ...
   SCIP_Real**           lpcandsfrac,        /**< pointer to store the array of LP candidate fractionalities, or NULL,zero as fractional value is possible */
   int*                  nlpcands,           /**< pointer to store the number of LP branching candidates, or NULL, LP values are possibly integral */
   int*                  npriolpcands        /**< pointer to store the number of candidates with maximal priority, or NULL, sol values are fractional */
   )
{
   SCIP_VAR** allvars;
   ...
   SCIP_CALL( SCIPgetVarsData(scip, &allvars, &nvars, &nbinvars, &nintvars, NULL, NULL) );
   nvars = nbinvars + nintvars;

   assert(lpcandsfrac != NULL);
   assert(lpcands != NULL);
   assert(npriolpcands != NULL);
   assert(nlpcands != NULL);
   *npriolpcands = 0;    
   *nlpcands = 0;
   
   SCIP_CALL( SCIPallocBufferArray(scip, lpcands, nintvars) ); 
   SCIP_CALL( SCIPallocBufferArray(scip, lpcandssol, nintvars) );
   SCIP_CALL( SCIPallocBufferArray(scip, lpcandsfrac, nintvars) );
  
   /* check each integral variable whether it shall be inserted into candidate array */
   for( j = 0; j < nintvars; ++j )
   {
      SCIP_VAR* var;
      SCIP_Real primsol;
         
      var = allvars[j+nbinvars];
      assert(var != NULL);

      primsol = SCIPvarGetLPSol(var);
      assert(primsol < SCIP_INVALID);
      assert(SCIPisFeasGE(scip, primsol, SCIPvarGetLbLocal(var)));
      assert(SCIPisFeasLE(scip, primsol, SCIPvarGetUbLocal(var)));
      ...
      frac = primsol - SCIPfeasFloor(scip, primsol);

      if( !SCIPisFeasEQ(scip, frac, 0.) )
      {
         /* put unfrac var to the end */
         (*lpcands)[*nlpcands] = (*lpcands)[*npriolpcands];
         (*lpcandssol)[*nlpcands] = (*lpcandssol)[*npriolpcands];
         (*lpcandsfrac)[*nlpcands] = (*lpcandsfrac)[*npriolpcands];
      
         /* put frac var to the first part */
         (*lpcands)[*npriolpcands] = var;
         (*lpcandssol)[*npriolpcands] = primsol;
         (*lpcandsfrac)[*npriolpcands] = frac;

         (*npriolpcands) += 1;
      } else {
         /* insert non-fractional variable at the end */
         (*lpcands)[*nlpcands] = var;
         (*lpcandssol)[*nlpcands] = primsol;
         (*lpcandsfrac)[*nlpcands] = frac;
      }

      (*nlpcands) += 1;
   }
   SCIPdebugMessage("npriolpcands = %d, nlpcands = %d\n",*npriolpcands, *nlpcands);
