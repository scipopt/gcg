...
#define READER_DESC             "file reader for MIPs in the LPB file format"
   ...
   )
{
   ...
   *var = SCIPfindVar(scip, name);
   if( *var == NULL )
   {
      ...
      SCIP_CALL( SCIPgetBoolParam(scip, "reading/lpbreader/dynamiccols", &dynamiccols) );
      initial = !dynamiccols;
   ...
   )
{
   ...
   *newsection = FALSE;
   ...
   /* read the first token, which may be the number of the pricing problem or the name of the line */
   if( getNextToken(lpbinput) )
   {
   ...
   )
{
   ...
   SCIP_CALL( SCIPgetBoolParam(scip, "reading/lpbreader/dynamicconss", &dynamicconss) );
   SCIP_CALL( SCIPgetBoolParam(scip, "reading/lpbreader/dynamicrows", &dynamicrows) );
