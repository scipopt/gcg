...
/**@file   heur_masterintdiving.c
 * @brief  master LP diving heuristic that fixes variables with integral LP value
...
#define HEUR_NAME             "masterintdiving"
#define HEUR_DESC             "master LP diving heuristic that fixes binary variables with large LP value to one"
...
#define heurCopyMasterintdiving NULL

/** destructor of primal heuristic to free user data (called when SCIP is exiting) */
static
SCIP_DECL_HEURFREE(heurFreeMasterintdiving) /*lint --e{715}*/
{  /*lint --e{715}*/
...
static
SCIP_DECL_HEURINIT(heurInitMasterintdiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXIT(heurExitMasterintdiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP_HEURDATA* heurdata;
...
static
SCIP_DECL_HEUREXEC(heurExecMasterintdiving) /*lint --e{715}*/
{  /*lint --e{715}*/
   SCIP* origprob;
   SCIP_HEURDATA* heurdata;
   ...
   *result = SCIP_DELAYED;
   ...
   SCIPdebugMessage("called Masterintdiving heuristic\n");
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( heurdata->maxpriceofs > -1 )
   {
      ...
      SCIPdebugMessage("masterintdiving - pricing rounds at this node: %d\n", npricerounds);
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   SCIPdebugMessage("(node %"SCIP_LONGINT_FORMAT") executing masterintdiving heuristic: depth=%d, %d non-fixed, dualbound=%g, searchbound=%g\n",
      ...
      && !SCIPisStopped(scip) )
   {
      ...
      do
      {
         ...
         if( !cutoff )
         {
            ...
            {
               SCIPwarningMessage(scip, "Error while solving LP in Masterfracdiving heuristic; LP solve terminated with code <%d>\n",retstat);
            }
#else
            nlpiterations = SCIPgetNLPIterations(scip);
            npricerounds = SCIPgetNPriceRounds(scip);
      ...
      if( !lperror && !cutoff && lpsolstat == SCIP_LPSOLSTAT_OPTIMAL )
      {
         ...
         if( nnewlpiterations > 0 || !SCIPisEQ(scip, bestsolval, bestfixval) )
         {
            ...
            if( success )
            {
               SCIPdebugMessage("masterintdiving found roundable primal solution: obj=%g\n",
   ...
   SCIPdebugMessage("(node %"SCIP_LONGINT_FORMAT") finished masterintdiving heuristic: %d fractionals, dive %d/%d, LP iter %"SCIP_LONGINT_FORMAT"/%"SCIP_LONGINT_FORMAT", objval=%g/%g, lpsolstat=%d, cutoff=%u\n",
         SCIPgetNNodes(scip), nfixcands, divedepth, maxdivedepth, heurdata->nlpiterations, maxnlpiterations,
...
/** creates the masterintdiving heuristic and includes it in SCIP */
SCIP_RETCODE SCIPincludeHeurMasterintdiving(
   ...
   )
{
   ...
   /* create Masterintdiving primal heuristic data */
   SCIP_CALL( SCIPallocMemory(scip, &heurdata) );

   /* include primal heuristic */
   SCIP_CALL( SCIPincludeHeurBasic(scip, &heur,
         HEUR_NAME, HEUR_DESC, HEUR_DISPCHAR, HEUR_PRIORITY, HEUR_FREQ, HEUR_FREQOFS,
         HEUR_MAXDEPTH, HEUR_TIMING, HEUR_USESSUBSCIP, heurExecMasterintdiving, heurdata) );

   assert(heur != NULL);

   /* set non-NULL pointers to callback methods */
   SCIP_CALL( SCIPsetHeurFree(scip, heur, heurFreeMasterintdiving) );
   SCIP_CALL( SCIPsetHeurInit(scip, heur, heurInitMasterintdiving) );
   SCIP_CALL( SCIPsetHeurExit(scip, heur, heurExitMasterintdiving) );

   /* masterintdiving heuristic parameters */
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/masterintdiving/minreldepth",
         "minimal relative depth to start diving",
         &heurdata->minreldepth, TRUE, DEFAULT_MINRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/masterintdiving/maxreldepth",
         "maximal relative depth to start diving",
         &heurdata->maxreldepth, TRUE, DEFAULT_MAXRELDEPTH, 0.0, 1.0, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/masterintdiving/maxlpiterquot",
         "maximal fraction of diving LP iterations compared to node LP iterations",
         &heurdata->maxlpiterquot, FALSE, DEFAULT_MAXLPITERQUOT, 0.0, SCIP_REAL_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddIntParam(scip,
         "heuristics/masterintdiving/maxlpiterofs",
         "additional number of allowed LP iterations",
         &heurdata->maxlpiterofs, FALSE, DEFAULT_MAXLPITEROFS, 0, INT_MAX, NULL, NULL) );
   SCIP_CALL( SCIPaddRealParam(scip,
         "heuristics/masterintdiving/maxpricequot",
