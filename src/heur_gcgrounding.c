...
{  /*lint --e{715}*/
   ...
   assert(scip != NULL);
   assert(result != NULL);
   assert(SCIPhasCurrentNodeLP(GCGrelaxGetMasterprob(scip)));
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   nlps = SCIPgetNLPs(GCGrelaxGetMasterprob(scip));
...
/** creates the gcgrounding heuristic with infeasibility recovering and includes it in SCIP */
