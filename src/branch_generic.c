...
)
{
   ...
   SCIP_CALL( SCIPallocMemoryArray(scip, &(record->record[record->recordsize]), 1) );
   for(i=0; i<Ssize;++i)
   {
   ...
   )
{
   ...
   SCIP_CALL( createChildNodesGeneric(origscip, NULL, S, Ssize, blocknr, F, Fsize, masterbranchcons, &nmasternodes) );// createorignodes) );
