...
If you compiled with CMake, you can use

```
make TEST=mytest SETTINGS=modified gcg_test
```
...
We assume that you have collected your runtime data using e.g. `make test` (CMake: `make gcg_test`). To then
generate a **report for the whole testset** including basic statistics and all
possible visualizations, you can call `make visu` (CMake: `make gcg_visu`). More details on the test set
...
`make test SETTINGS=settings1` and `make test SETTINGS=settings2` (CMake: `make gcg_test ...`) and have your runtime data
...
and **a `make test` for your custom version of GCG** (CMake: `make gcg_test`) and have your runtime data
...
### Automated Version Comparison Script (Makefile Build only)
...
### Manual Version Comparison (Makefile Build only)
