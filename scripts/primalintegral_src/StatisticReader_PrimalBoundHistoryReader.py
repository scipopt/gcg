...
class PrimalBoundHistoryReader(StatisticReader):
   ...
   regular_exp_gcg = '  time | node  | left  |SLP iter|MLP iter|LP it/n| mem |mdpt |mvars|ocons|mcons|mcuts|sepa|  dualbound   | primalbound  |  deg   |  gap   '
   columnwidth = 20
   ...
   #         print pointInTime, PrimalBound
      ...
      if StatisticReader.solvertype == StatisticReader.SOLVERTYPE_GCG:
         if self.regular_exp_gcg in line:
            self.inTable = True
         elif self.inTable and re.search("s\|", line):
   #         print line
            mylist = line.split()
   #         print mylist
            ...
            if re.search("\|",mylist[len(mylist) - 1]):
               if re.search("\%\|", mylist[len(mylist) - 1]):
                  ...
                  PrimalBound = mylist[len(mylist) - 4].strip("|")
            else:
               if re.search("\%\|", mylist[len(mylist) - 2]):
                  PrimalBound = mylist[len(mylist) - 4].strip("|")
               else:
                  PrimalBound = mylist[len(mylist) - 5].strip("|")
   #         print pointInTime, PrimalBound
            if PrimalBound != self.lastPrimalBound:
               self.lastPrimalBound = PrimalBound
               self.listOfPoints.append((float(pointInTime),float(PrimalBound)))
