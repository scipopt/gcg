...
/^@01 / {
   ...
   if( onlygroup == 0 || setingroup[myname,onlygroup] )
   {
      solvername[nsolver] = myname;
      nsolver++;
   }
   nprobs[nsolver] = 0;
}
...
// {
   ...
   if( $33 in statuses ) # GCG Jonas
   {
      # collect data (line with problem type, original and presolved problem size and simplex iterations)
      type[nsolver,nprobs[nsolver]] = $2;
      conss[nsolver,nprobs[nsolver]] = $5;
      vars[nsolver,nprobs[nsolver]] = $6;
      rootdbs[nsolver,nprobs[nsolver]] = max(min($10, +infinity), -infinity);
      ...
      btime[nsolver,nprobs[nsolver]] = fracceil(max($22,mintime),0.1);
      bcuts[nsolver,nprobs[nsolver]] = $25;
      status[nsolver,nprobs[nsolver]] = $33;
      validline = 1;
   }
...
END {
   ...
   for( s = 0; s < nsolver; ++s )
   {
      ...
      for( cat = -1; cat <= 3; cat++ )
      {
         ...
         bcuttotal[s,cat] = 0.0;
         bcutgeom[s,cat] = 1.0;
         bcutshiftedgeom[s,cat] = bcutgeomshift;
         refbcuttotal[s,cat] = 0.0;
         refbcutgeom[s,cat] = 1.0;
         refbcutshiftedgeom[s,cat] = bcutgeomshift;

         rootdbtotal[s,cat] = 0.0;
         rootdbgeom[s,cat] = 1.0;
         rootdbshiftedgeom[s,cat] = rootdbgeomshift;
         refrootdbtotal[s,cat] = 0.0;
   ...
   bestbcutgeom = 1.0;
   bestbcutshiftedgeom = bcutgeomshift;
...
#      for( o = 0; o < s; ++o )
   ...
   if( texcmpfile != "" )
   {
      ...
      for( o = 0; o < nsolver; ++o )
      {
         s = printorder[o];
         solverextension = solverextension "i";
         printf("& \\multicolumn{4}{@{\\spc}c%s}{\\solvername%s} ", o < nsolver-1 ? "@{\\spc}" : "", solverextension) > texcmpfile;
      }
      printf("\\\\\n") > texcmpfile;
      for( o = 0; o < nsolver; ++o )
   ...
   for( i = 0; i < problistlen; ++i )
   {
      ...
      for( s = 0; s < nsolver; ++s )
      {
         ...
         bcuts[s,pidx] = max(bcuts[s,pidx], 0.0);

         #rootdbs[s,pidx] = max(rootdbs[s,pidx], 0.0);
         ...
         if( processed && (status[s,pidx] == "ok" || status[s,pidx] == "unknown" || status[s,pidx] == "timeout" || status[s,pidx] == "nodelimit" || status[s,pidx] == "memlimit") )
         {
            ...
            bestbcuts = min(bestbcuts, bcuts[s,pidx]);
            worstbcuts = max(worstbcuts, bcuts[s,pidx]);
      ...
      worstbcuts = max(worstbcuts, 0.0);

      #worstrootdbs = max(worstrootdbs, 1.0);
          ...
          (!onlyfail || fail) )
      {
         ...
         if( texcmpfile != "" )
         {
            ...
            refbcuts = bcuts[ref,probidx[p,ref]];
            ...
            for( o = 0; o < nsolver; ++o )
            {
               ...
               if( bcuts[s,pidx] < 0.5*refbcuts && !hitlimit[s,pidx] )
                  bcutcolor = "red";
               else if( bcuts[s,pidx] > 2.0*refbcuts && !refhitlimit )
                  bcutcolor = "blue";
               else
                  bcutcolor = "black";

               if( rootdbs[s,pidx] < 0.5*refrootdbs && !hitlimit[s,pidx] )
                  rootdbcolor = "blue";
               else if( rootdbs[s,pidx] > 2.0*refrootdbs && !refhitlimit )
                  rootdbcolor = "red";
               else
                  rootdbcolor = "black";
          ...
          (!onlyfeasible || hasfeasible) && (!onlyinfeasible || !hasfeasible) )
      {
         ...
         bestbcutgeom = bestbcutgeom^((nevalprobs[s,0]-1)/nevalprobs[s,0]) * bestbcuts^(1.0/nevalprobs[s,0]);
         bestbcutshiftedgeom = bestbcutshiftedgeom^((nevalprobs[s,0]-1)/nevalprobs[s,0]) * (bestbcuts+bcutgeomshift)^(1.0/nevalprobs[s,0]);
   ...
   for( s = 0; s < nsolver; ++s )
   {
      for( cat = -1; cat <= 3; cat++ )
      {
         ...
         #bcuttotal[s,cat] = max(bcuttotal[s,cat], 1);
         #refbcuttotal[s,cat] = max(refbcuttotal[s,cat], 1);

         #gapclosedtotal[s,cat] = max(gapclosedtotal[s,cat], 1);
         #refgapclosedtotal[s,cat] = max(refgapclosedtotal[s,cat], 1);
      }
   }
   ...
   for( o = 0; o < nsolver; ++o )
   {
      ...
      if( o == 0 || short )
      {
         ...
         if( bcutgeomcomp < 0 )
            bcutgeomcomp = bcutgeom[s,0];
         if( bcuttotalcomp < 0 )
            bcuttotalcomp = bcuttotal[s,0];
   ...
   for( o = 0; o < nsolver; ++o )
   {
      ...
      for( cat = -1; cat <= 3; cat++ )
      {
         ...
         bcutshiftedgeom[s,cat] -= bcutgeomshift;
         #bcutshiftedgeom[s,cat] = max(bcutshiftedgeom[s,cat], 1);
         refbcutshiftedgeom[s,cat] -= bcutgeomshift;
         #refbcutshiftedgeom[s,cat] = max(refbcutshiftedgeom[s,cat], mintime);
      ...
      if( o == 0 || short )
      {
         ...
         if( bcutshiftedgeomcomp < 0 )
            bcutshiftedgeomcomp = bcutshiftedgeom[s,0];
   ...
   bestbcutshiftedgeom -= bcutgeomshift;
   #bestbcutshiftedgeom = max(bestbcutshiftedgeom, 1.0);
...
#            continue;
         ...
         for( o = 0; o < nsolver; ++o )
         {
            ...
            if( o == 0 )
            {
               ...
               bcutgeomcomp = bcutgeom[s,cat];
               bcutshiftedgeomcomp = bcutshiftedgeom[s,cat];
 ...
 #                 bcutgeom[s,cat], bcutshiftedgeom[s,cat], bcutgeom[s,cat]/refbcutgeom[s,cat],
  #                bcutshiftedgeom[s,cat]/refbcutshiftedgeom[s,cat],
            }
         }
         if( cat == 0 )
         {
   ...
   if( texfile != "" )
   {
      ...
      printf("\\begin{tabular*}{\\columnwidth}{@{\\extracolsep{\\fill}}lrrr@{\\spc}rrrr@{\\spc}rrrr") > texfile;
      if( hasequalpath )
   ...
   if( texsummaryfile != "" )
   {
      ...
      if( texsummaryheader > 0 )
      {
         ...
         for( si = 0; si <= 4; si++ )
         {
            ...
            {
               printf("\\midrule\n") > texsummaryfile;
               printf("\\input{Tables/mip/auto/%s_bcuts}\n", texsummarybase) > texsummaryfile;
            }
            if( si == 0 || si == 4 )
            {
               printf("\\midrule\n") > texsummaryfile;
      ...
      printf("& \\testset{%s}", textestset) >> texsummaryfilebcuts;
      for( o = 1; o < nsolver; o++ )
      {
         s = printorder[o];
         if( texsummaryshifted )
            printf(" & %s", texcompstr(bcutshiftedgeom[s,0], refbcutshiftedgeom[s,0])) > texsummaryfilebcuts;
         else
            printf(" & %s", texcompstr(bcutgeom[s,0], refbcutgeom[s,0])) > texsummaryfilebcuts;
      }
      printf("\\\\\n") > texsummaryfilebcuts;
