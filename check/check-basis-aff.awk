...
# time
#
/^Solving Time \(sec\) :/ { tottime = $5 } # for interrupted solving (no statistics)
...
/^=ready=/ {
   ...
   if( !headerprinted ) {
      ...
      if (TEXFILE != "") {
         ...
         printf("\\tablehead{\n\\toprule\n")                              >TEXFILE;
         printf("Name             & Conss &  Vars & root db & Dual Bd & Primal Bd & Gap\\%% &  Calls &  Probs &  Vars & P-Time & LP-Time &  Nodes &  Time  &b:Time & Calls & Cuts & Applied & Convex & DiffStart & DiffEnd &m:Time & Calls & Cuts & Applied") > TEXFILE;
         if( printsoltimes )
	 ...
	 printf("\\hspace{-1cm}\n")					  >TEXFILE;
         printf("\\begin{supertabular}{@{\\extracolsep{\\fill}}lrrrrrrrrrrrrrrrrrrrrrrrrrr") >TEXFILE;
         if( printsoltimes )
            printf("rr") > TEXFILE;
         printf("@{}}\n") > TEXFILE;
      }
      ...
      tablehead2 = "Name       |Type| Conss |  Vars | Conss |  Vars |Blocks| Rel  |MConss|root db|Dual Bd|Primal Bd| Gap%|Calls|Probs| Vars| Time|LP-Time|  Iters|Nodes|Time|b:Time|Calls|Cuts|Appl|Conv|DifSt|DifEnd|m:Time|Calls|Cuts| Appl";
      tablehead3 = "-----------+----+-------+-------+-------+-------+------+------+------+--------------+--------------+------+-------+-------+-------+-------+-------+--------+-------+-------+------------+-------+------+---------+---------+--------";
   ...
   if( (!onlyinsolufile || solstatus[prob] != "") &&
	 ...
	 if( timeout || gapreached || sollimitreached || memlimitreached || nodelimitreached ) {
      ...
      if( !onlypresolvereductions || origcons > cons || origvars > vars ) {
         if (TEXFILE != "") {
            printf("%-16s & %6d & %6d & %5.1f & %5.1f & %5.1f & %6s &  %6d &  %5d &  %5d & %5.1f & %5.1f & %s%8d &%s%5.1f & %5.1f & %5d & %5d & %5d & %5.3f & %5.3f & %5.3f & %5.1f & %5d & %5d & %5d ",
                   pprob, cons, vars, rootdb, db, pb, gapstr, pricecall, npriceprobs, pricevars, pricetime, lptime,
                   markersym, bbnodes, markersym, tottime, basetime, basecalls, basefound, baseapplied, baseconvex, basediffstart, basediffend, mastertime, mastercalls, masterfound, masterapplied) >TEXFILE;
            if( printsoltimes )
               printf(" & %7.1f & %7.1f", timetofirst, timetobest) > TEXFILE;
            printf("\\\\\n") > TEXFILE;
         }
         printf("%-12s %-3s %7d %7d %7d %7d %6d %6d %6d %5.1f %5.4f %5.4f %6s %5d %5d %5d %5.1f %5.1f %5d %5d %5.1f %5.1f %5d %5d %5d %6.6f %5.3f %5.3f %5.1f %5d %5d %5d ",
                shortprob, probtype, origcons, origvars, cons, vars, blocks, rel, mcons, rootdb, db, pb, gapstr, 
                pricecall, npriceprobs, pricevars, pricetime, lptime, simpiters, bbnodes, tottime, 
		basetime, basecalls, basefound, baseapplied, baseconvex, basediffstart, basediffend, mastertime, mastercalls, masterfound,
		masterapplied);
         if( printsoltimes )
...
END {
   ...
   if (TEXFILE != "" ) {
      printf("\\midrule\n")                                                 >TEXFILE;
      printf("%-11s (%2d) &     &    & %5.4g &      &       &       & %7d & %7d & %7d &%5.1f &%5.1f &  %5d & %5.1f & %5.1f & %5d & %5d & %5d & %5.3f & %5.3f & %5.3f & %5.1f & %5d & %5d & %5d ",
             "Total", nprobs, srootdb, spricecalls, spriceprobs, spricevars, spricetime, slptime, sbab, stottime, sbasetime, sbasecalls, sbasefound, sbaseapplied, sbaseconvex, sbasediffstart, sbasediffend, smastertime, smastercalls, smasterfound, smasterapplied) >TEXFILE;
      if( printsoltimes )
         printf(" & %8.1f & %8.1f", stimetofirst, stimetobest) > TEXFILE;
      printf("\\\\\n") > TEXFILE;
      printf("%-11s      &     &     & %5.4f &      &      &      & %7d & %7d & %7d &%5.1f &%5.1f &  %5.1f & %5.1f & %5.1f & %5.1f & %5.1f & %5.1f & %6.6f & %5.3f & %5.3f & %5.1f & %5.1f & %5.1f & %5.1f ",
             "Geom. Mean", rootdbgeom, pricecallsgeom, priceprobsgeom, pricevarsgeom, pricegeom, lpgeom, nodegeom, timegeom, 
		basetimegeom, basecallsgeom, basefoundgeom, baseappliedgeom, baseconvexgeom, basediffstartgeom, basediffendgeom, mastertimegeom, mastercallsgeom, masterfoundgeom, masterappliedgeom) >TEXFILE;
      if( printsoltimes )
         printf(" & %8.1f & %8.1f", timetofirstgeom, timetobestgeom) > TEXFILE;
      printf("\\\\\n") > TEXFILE;
      printf("%-13s    &     &     &  %5.4f &     &     &    & %7d & %7d & %7d &%5.1f &%5.1f &  %5.1f & %5.1f & %5.1f & %5.1f & %5.1f & %5.1f & %6.6f & %5.3f & %5.3f & %5.1f & %5.1f & %5.1f & %5.1f ",
             "Shifted Geom.", shiftedrootdbgeom, shiftedpricecallsgeom, shiftedpriceprobsgeom, shiftedpricevarsgeom, shiftedpricegeom, shiftedlpgeom, shiftednodegeom, shiftedtimegeom, 
		shiftedbasetimegeom, shiftedbasecallsgeom, shiftedbasefoundgeom, shiftedbaseappliedgeom, shiftedbaseconvexgeom,
		shiftedbasediffstartgeom, shiftedbasediffendgeom,
	shiftedmastertimegeom, shiftedmastercallsgeom, shiftedmasterfoundgeom, shiftedmasterappliedgeom) >TEXFILE;
      if( printsoltimes )
